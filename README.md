...
```text
logger.Info("msg") → zap 编码 → lumberjack.Write() → os.File.Write() → 内核缓冲区 → 磁盘
...
```text
logger.Info("msg") → zap 编码 → BufferedWriteSyncer.Write() → 内存缓冲区（用户态）
                                                                    │
                              缓冲区满 或 定时器到期 ──────────────────┘
                                                                    ↓
...
defer logger.Sync() // 重要：确保退出时 flush 缓冲区
```
